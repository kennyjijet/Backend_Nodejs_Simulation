...
var eventTemp = [];
module.exports = function (user0, user1, matchType) {
    createfootballPlayerStats(user0, user1);
    soccer_field_gen.createFootballPlayersPosition();
    ///summarizedZone(IsHome) 
    ...
    var teams = matchResultCalculation(footballPlayersBothActionsStats);
    ...
    teams[0].Goal = statsHome.score;
    teams[0].Lose = statsAway.score;
    teams[1].Goal = statsAway.score;
    teams[1].Lose = statsHome.score;
...
function simulationSoccerGame(matchType) {
...
function passingBallEvent() {
    ...
    console.log("passing" + actionType);
...
function updateRelationShips(listFootballPlayers, footballPlayerStatsArray) {                           
    ...
    */ 
    /*
        talent 
        friendly -> add extra value of human relation 20%. 0.5 + 20%
        quiet person -> minus value of human relation 20%. 0.5 - 20%
     */
...
function injuryChanceOfFootBallplayerImpact(ballPossessionHome, actionType, mainPlayer, subPlayer) {
    /*
        talent 
        endure -> decrease 10% of condition reduction.
        flexibility -> decrease injury rate 3%.
         
        rickety -> add extra value of injury 20% to footballplayer; 
        fatigue -> increase 10% of condition reduction. 
        badluck -> increase injury rate 3%
